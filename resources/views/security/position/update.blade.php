...
@section('content_header')
    <h1 class="display">Actualizacion de Cargo</h1>
    <hr>
...
@section('content')
    <form action="{{route('security.position.update.post',$position->id)}}" method="post" class="row g-3">
        @csrf
        <div class="col-md-12">
            <label for="name" class="form-label">Nombre del Cargo</label>
            <input value="{{$position->name}}" maxlength="64" type="text" name="name" id="name" class="form-control" placeholder="Nombre">
        </div>
        <div class="col-md-12">
            <label for="description" class="form-label">Descripcion</label>
            <textarea maxlength="256" style="resize:none;" class="form-control" id="description" name="description" rows="4">{{$position->description}}</textarea>
        </div>
        <div class="col-12">
