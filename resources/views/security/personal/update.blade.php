    ...
    <div class="pt-3">
        <div class="p-0 d-flex flex-column main-content ">
            <div class="container-fluid bg-dark d-flex flex-column container-head">
                ...
                <div class=" d-flex gap-0 pt-5 pb-5 flex-column">
                    <h1 class="text-center">
                        <img class="rounded-circle" width="156" height="156" src="{{asset('storage/personal_photos/photo')}}-{{$personal->identification}}.png" alt="">
                    </h1>
                    <h5 class="text-center p-0 m-0">{{$personal->name}} {{$personal->last_name}}</h5>
                    <h5 style="color:gray;" class="text-center p-0 m-0">{{$personal->identification}}</h5>
                </div>
                <div>
                    <div class="d-flex flex-column pb-0 p-2">
                        <h5>Actualizacion de Personal</h5>
                    </div>
                </div> 
            </div>
            <div class=" rounded-bottom container-fluid form-body bg-dark ">
                <form action="{{route('security.personal.update.post',$personal->personal_id)}}" method="post" class="row g-3 pl-2 pr-2 pb-4 pt-2" enctype="multipart/form-data">
                    @csrf
                    <div class="col-md-6">
                        <input value="{{$personal->name}}" maxlength="64" type="text" name="Nombre" id="name" class="form-control" placeholder="Nombre">
                    </div>
                    <div class="col-md-6">
                        <input value="{{$personal->last_name}}" maxlength="64" type="text" name="Apellido" id="last_name" class="form-control" placeholder="Apellido">
                    </div>
                    <div class="col-md-12">
                        <input value="{{$personal->identification}}" type="number" name="Identificacion" id="identification" class="form-control" placeholder="Identificacion">
                    </div>
                    <div class="col-md-12">
