...
@section('content')
    <form action="{{route('security.area.update.post',$area->id)}}" method="post" class="row g-3">
        @csrf
        <div class="col-md-12">
            <label for="name" class="form-label">Nombre del Area</label>
            <input value="{{$area->name}}" maxlength="64" type="text" name="name" id="name" class="form-control" placeholder="Nombre">
        </div>
        <div class="col-md-12">
            <label for="description" class="form-label">Descripcion</label>
            <textarea maxlength="256" style="resize:none;" class="form-control" id="description" name="description" rows="4">{{$area->description}}</textarea>
        </div>
        <div class="col-12">
