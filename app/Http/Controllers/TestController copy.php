...
class TestController extends Controller
{
    public function index(){
        ...
        return view('test', compact('number', 'start', 'end','date','data'));
    }
    ...
    private function allData($date, $allData = array())
    {
        ...
        $allData["month"][0]  = $this->get_month_data($date,0,0);
        $allData["month"][1]  = $this->get_month_data($date,0,1);
        $allData["month"][2]  = $this->get_month_data($date,0,2);
        $allData["year"]      = $this->get_year_data($date);
        $allData["types"]     = $this->get_types_data($date);
    ...
    private function get_today_data($date)
    {
        ...
        $data["HAR"] = XmlHistoryData::where('report_id', $report)->where('heading_id', 2)->value('day') - XmlHistoryData::where('report_id', $report)->where('heading_id', 6)->value('day') - XmlHistoryData::where('report_id', $report)->where('heading_id', 7)->value('day') - XmlHistoryData::where('report_id', $report)->where('heading_id', 124)->value('day');

        return $data;
    }

    private function get_week_data($date, $i = 0, $j = 1)
    {
    ...
    private function get_month_data($date, $i = 0, $j = 0)
    {
        $data = array(
            "name"    => $this->get_name_month(date("m", strtotime(date($date)."+" . $j . "month"))),
            "history" => array(
        ...
        $days  = date("t", strtotime(date($date)."+" . $j . "month"))-1;
        $month = date("Y-m", strtotime(date($date)."+" . $j . "month"));
    ...
    private function get_types_data($date, $i = 0, $j = 124)
    {
        $report = XmlHistoryReport::where('date', $date)->value('id');
        ...
        while ($j <= 278) {
            $validation = XmlHistoryData::where('report_id', $report)->where('heading_id', $j)->value('day');
            if ($validation) {
                $data[$i]["name"]     = $this->get_name_type($j);
                $data[$i]["rooms"]    = XmlHistoryData::where('report_id', $report)->where('heading_id', $j++)->value('day');
                $data[$i]["adults"]   = XmlHistoryData::where('report_id', $report)->where('heading_id', $j++)->value('day');
                $data[$i]["childrem"] = XmlHistoryData::where('report_id', $report)->where('heading_id', $j++)->value('day');
                $data[$i]["revenue"]  = XmlHistoryData::where('report_id', $report)->where('heading_id', $j++)->value('day');
                $i++;$j++;
            }else{
                $name = $this->get_name_type($j);
                switch ($name) {
                    case 'CMP':
                        if (XmlHistoryData::where('report_id', $report)->where('heading_id', 6)->value('day')) {
                            $data[$i]["name"]     = $name;
                            $data[$i]["rooms"]    = XmlHistoryData::where('report_id', $report)->where('heading_id', 6)  ->value('day');
                            $data[$i]["adults"]   = XmlHistoryData::where('report_id', $report)->where('heading_id', 125)->value('day');
                            $data[$i]["childrem"] = XmlHistoryData::where('report_id', $report)->where('heading_id', 126)->value('day');
                            $data[$i]["revenue"]  = XmlHistoryData::where('report_id', $report)->where('heading_id', 127)->value('day');
                            $i++;
                        }
                    break;
                    
                    case 'HSU':
                        if (XmlHistoryData::where('report_id', $report)->where('heading_id', 7)->value('day')) {
                            $data[$i]["name"]     = $name;
                            $data[$i]["rooms"]    = XmlHistoryData::where('report_id', $report)->where('heading_id', 7)  ->value('day');
                            $data[$i]["adults"]   = XmlHistoryData::where('report_id', $report)->where('heading_id', 135)->value('day');
                            $data[$i]["childrem"] = XmlHistoryData::where('report_id', $report)->where('heading_id', 136)->value('day');
                            $data[$i]["revenue"]  = XmlHistoryData::where('report_id', $report)->where('heading_id', 137)->value('day');
                            $i++;
                        }
                    break;

                    case 'GCP':
                        if (XmlHistoryData::where('report_id', $report)->where('heading_id', 22)->value('day')) {
                            $data[$i]["name"]     = $name;
                            $data[$i]["rooms"]    = XmlHistoryData::where('report_id', $report)->where('heading_id', 22) ->value('day');
                            $data[$i]["adults"]   = XmlHistoryData::where('report_id', $report)->where('heading_id', 220)->value('day');
                            $data[$i]["childrem"] = XmlHistoryData::where('report_id', $report)->where('heading_id', 221)->value('day');
                            $data[$i]["revenue"]  = XmlHistoryData::where('report_id', $report)->where('heading_id', 222)->value('day');
                            $i++;
                        }
                    break;

                    case 'WHD':
                        if (XmlHistoryData::where('report_id', $report)->where('heading_id', 25)->value('day')) {
                            $data[$i]["name"]     = $name;
                            $data[$i]["rooms"]    = XmlHistoryData::where('report_id', $report)->where('heading_id', 25) ->value('day');
                            $data[$i]["adults"]   = XmlHistoryData::where('report_id', $report)->where('heading_id', 155)->value('day');
                            $data[$i]["childrem"] = XmlHistoryData::where('report_id', $report)->where('heading_id', 156)->value('day');
                            $data[$i]["revenue"]  = XmlHistoryData::where('report_id', $report)->where('heading_id', 109)->value('day');
                            $i++;
                        }
                    break;

                    case 'CNT':
                        if (XmlHistoryData::where('report_id', $report)->where('heading_id', 118)->value('day')) {
                            $data[$i]["name"]     = $name;
                            $data[$i]["rooms"]    = XmlHistoryData::where('report_id', $report)->where('heading_id', 118)->value('day');
                            $data[$i]["adults"]   = XmlHistoryData::where('report_id', $report)->where('heading_id', 125)->value('day');
                            $data[$i]["childrem"] = XmlHistoryData::where('report_id', $report)->where('heading_id', 126)->value('day');
                            $data[$i]["revenue"]  = XmlHistoryData::where('report_id', $report)->where('heading_id', 127)->value('day');
                            $i++;
        ...
        foreach ($data as $key => $value) {
            $data[$key]["per"]     = $value["rooms"] / XmlHistoryData::where('report_id', $report)->where('heading_id', 3)->value('day') * 100;
