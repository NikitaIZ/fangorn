...
class RoomTask extends Command
{
    /**
     ...
     * @var string
     */
    protected $description = 'Actualizar reporte del rooms';
    ...
    private function updateXmlHistoryReport($folder, $array){
        $id = XmlHistoryReport::where('folder', $folder)->value('id');
        foreach ($array as $key => $dato){
            $XmlHistoryData = new XmlHistoryData();
            $XmlHistoryData->report_id  = $id;
            ...
            if (array_key_exists('YEAR', $dato)) {
                $XmlHistoryData->month  = $dato['YEAR'];
            }
            $XmlHistoryData->save();
        }
    }

    private function orderXml($urlBuc, $urlJournal, $name, $all_data = array()){

        $folder = $this->orderDateFiles($name);
        $report_id = XmlHistoryReport::where('folder', $folder)->value('id');
        $total_rooms = XmlHistoryData::where('report_id', $report_id)->where('heading_id', 1)->value('day');
        ...
        $all_data['TS_ADULTS_IN_HOUSE']['DAY']     = $types['CNT']['Adults'];
        $all_data['TS_CHILDREN_IN_HOUSE']['DAY']   = $types['CNT']['Childrem'];
        $all_data['TS_REVENUE']['DAY']             = $types['CNT']['Revenue'];

        $all_data['WHD_ADULTS_IN_HOUSE']['DAY']   = $types['WHD']['Adults'];
        $all_data['WHD_CHILDREN_IN_HOUSE']['DAY'] = $types['WHD']['Childrem'];
        $all_data['OCC_PERC_WHD']['DAY']          = $types['WHD']['Rooms'] / $total_rooms * 100;

        $all_data['COMP_ADULTS_IN_HOUSE']['DAY']   = $types['CMP']['Adults'];
        $all_data['COMP_CHILDREN_IN_HOUSE']['DAY'] = $types['CMP']['Childrem'];
        $all_data['COMP_REVENUE']['DAY']           = $types['CMP']['Revenue'];
        $all_data['OCC_PERC_COMP']['DAY']          = $types['CMP']['Rooms'] / $total_rooms * 100;

        $all_data['HSU_ADULTS_IN_HOUSE']['DAY']   = $types['HSU']['Adults'];
        $all_data['HSU_CHILDREN_IN_HOUSE']['DAY'] = $types['HSU']['Childrem'];
        $all_data['HSU_REVENUE']['DAY']           = $types['HSU']['Revenue'];
        $all_data['OCC_PERC_HSU']['DAY']          = $types['HSU']['Rooms'] / $total_rooms * 100;

        $all_data['GCP_ADULTS_IN_HOUSE']['DAY']   = $types['GCP']['Adults'];
        $all_data['GCP_CHILDREN_IN_HOUSE']['DAY'] = $types['GCP']['Childrem'];
        $all_data['GCP_REVENUE']['DAY']           = $types['GCP']['Revenue'];
        $all_data['OCC_PERC_GCP']['DAY']          = $types['GCP']['Rooms'] / $total_rooms * 100;

        foreach ($types as $key => $value) {
            if ($key != 'CNT' && $key != 'CMP' && $key != 'HSU' && $key != 'WHD' && $key != 'GCP') {
                if ($value['Rooms'] > 0) {
                    ...
                    $all_data['OCC_PERC_' . $key]['DAY']          = $value['Rooms'] / $total_rooms * 100;
                }
            }
        }
    ...
    private function getDate($folder){
    ...
    {
        $reports = XmlHistoryReport::orderBy('id', 'desc')->get();
        foreach ($reports as $value) {
            $date = $this->getDate($value->folder);
            $condition = XmlHistoryData::where('report_id', $value->report_id)->where('heading_id', 124)->value('day');
            if ($condition == null) {
                $fileBucChk  = "M:\\". $date ."\buc_chk_exc.xml";
                $fileJournal = "M:\\". $date ."\journal.xml";
                if (file_exists($fileBucChk) == true AND file_exists($fileJournal) == true) {
                    $this->orderXml($fileBucChk, $fileJournal, $date);
                }
            }
        }
    }
